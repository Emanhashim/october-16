...
public class AdminSettings {
    ...
    public ResponseEntity<?> getNumberOfAgents(Authentication authentication) {
		...
		if (!agentRoleOptional.isPresent()) {
			return ResponseEntity.badRequest().body(new ResponseError("Account Type AGENT Does not Exist"));
		}
		if ( agentRepository.findByroles(agentRoleOptional.get()).size()==0) {
			return ResponseEntity.badRequest().body(new ResponseError("No Registered Agent"));
		}
		List<AgentInfo> agentInfo = agentRepository.findByroles(agentRoleOptional.get());
    ...
    public ResponseEntity<?> getNumberOfMerchants(Authentication authentication) {
		...
		Optional<Role> merchantRoleOptional = roleRepository.findByrolename("MERCHANT");
		if (!merchantRoleOptional.isPresent()) {
			return ResponseEntity.badRequest().body(new ResponseError("Account Type MERCHANT Does not Exist"));
		}
		if ( agentRepository.findByroles(merchantRoleOptional.get()).size()==0) {
			return ResponseEntity.badRequest().body(new ResponseError("No Registered Agent"));
		}
		List<AgentInfo> merchanInfo = agentRepository.findByroles(merchantRoleOptional.get());
		return ResponseEntity.ok(new TotalResponseMerchants(merchanInfo.size(),"Total number of registered Merchants: "+merchanInfo.size()));
